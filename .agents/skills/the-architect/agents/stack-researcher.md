...
8. **Never edit files.** Even if you notice a wrong pin in
   `${CLAUDE_PLUGIN_ROOT}/knowledge/runtime-tracks/*.md` — report it, do not fix it.
...
Use `Read`/`Grep` on `${CLAUDE_PLUGIN_ROOT}/knowledge/runtime-tracks/` to see what is currently
...
- `${CLAUDE_PLUGIN_ROOT}/knowledge/runtime-tracks/ts-node.md` — the only place pins are allowed to live
- `${CLAUDE_PLUGIN_ROOT}/knowledge/stack-compatibility.md` — known-bad combinations, checked after versions resolve
- `${CLAUDE_PLUGIN_ROOT}/agents/blueprint-writer.md` — the consumer of this report
- `${CLAUDE_PLUGIN_ROOT}/commands/architect-refresh.md` — the refresh workflow that drives this agent
